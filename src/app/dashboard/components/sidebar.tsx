...
import { LayoutDashboard, Users, Video, Layers, Users2, Settings, ChevronDown, Menu, Clapperboard } from "lucide-react"
        ...
        <nav className="space-y-2">
          ...
          <Accordion type="single" collapsible className="w-full border-none">
            <AccordionItem value="content" className={cn(
              ...
              <AccordionContent className="pb- pt-0">
                <div className={cn(
                  "relative space-y-1 border-l-2 mt-2",
                  "border-slate-300 dark:border-slate-700/50"
                )}>
                  <NavItem icon={<Video size={18} />} link="" label="Videos" />
                  <NavItem icon={<Layers size={18} />} link="" label="Series" />
                  <NavItem icon={<Users2 size={18} />} link="" label="Small Group" />
                </div>
              </AccordionContent>
            </AccordionItem>
          </Accordion>
...
function NavItem({
  ...
  return (
    <Link href={link} className={cn(
      "flex items-center gap-3 px-3 py-2 rounded-xl cursor-pointer duration-200 group",
      active
