...
import { X, Upload, FileVideo, Loader2, CheckCircle2 } from "lucide-react";
...
export default function UploadEpisodeModal({ seriesId, seriesTitle, open, onClose }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Response shape: { data: { sessions: [{ video: { id }, upload: { uploadLink } }] } }
    // Also handle flat array: { data: [{ video: { id }, upload: { uploadLink } }] }
    const inner = sessionRes.data?.data;
    const sessions = inner?.sessions ?? (Array.isArray(inner) ? inner : null);
  ...
  return (
    ...
    >
      <div className="w-full max-w-xl bg-white dark:bg-[#111318] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-white/5">
          <div>
            <h2 className="text-base font-bold text-slate-900 dark:text-white">Upload Episode</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 truncate max-w-xs">{seriesTitle}</p>
          </div>
          <button
            type="button"
            onClick={loading ? handleCancelUpload : handleClose}
