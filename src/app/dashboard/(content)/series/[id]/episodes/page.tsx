...
import { getSeriesDetailAction, getSeriesVideosAction } from "@/app/loadAction";
...
export default function SeriesEpisodesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [detailRes, videosRes] = await Promise.all([
      getSeriesDetailAction(seriesId),
      getSeriesVideosAction(seriesId),
    ]);

    if (detailRes.success) {
      const s = detailRes.data?.data ?? detailRes.data;
      setSeries(s);
    } else {
      toast({ type: "error", title: "Failed to load", message: "Could not load series details." });
    }
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-white dark:bg-[#111318] rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
          <div className="flex items-center gap-4 p-6">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-xl font-bold text-slate-900 dark:text-white">{series.title}</h1>
              {series.description && (
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-2">{series.description}</p>
              )}
        ...
        {episodes.length === 0 ? (
          <div className="bg-white dark:bg-[#111318] rounded-2xl border border-slate-200 dark:border-slate-800 p-16 text-center">
            ...
            <p className="text-sm text-slate-500 mt-1">Upload episodes from the Series list page.</p>
          </div>
        ) : (
          <div className="bg-white dark:bg-[#111318] rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
